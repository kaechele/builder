...
test('Navigates to child-level page', async ({ page }) => {
  ...
  await expect(page.url()).toBe(
    'http://localhost:5173/theme-nonprofit/blog/blog-post',
  );
});
...
test('Edits Site CSS', async ({ page }) => {
  ...
  await page
    .getByText(
      '@import url("https://unpkg.com/@primo-app/primo@1.3.64/reset.css");#page { font-',
    )
    .fill(
...
test('Creates a new page', async ({ page }) => {
  ...
  await page
    .getByRole('combobox', { name: 'Create from' })
    .selectOption('null');
  await page
    .getByRole('listitem')
    .filter({
      hasText:
        'Page Name Page URL Create from Blank───────AboutMissionHome PageTeamBlogBlog Pos',
    })
    .getByRole('button')
    .click();
  const detailsElement = await page.$(
    '.details:has-text("blankpage /blankpage")',
  );
