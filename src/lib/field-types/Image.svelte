<script>
  ...
  async function convert_image(url, new_type) {
    ...
    image.onload = () => {
      ...
      canvas.toBlob((blob) => {
        ...
        const myImage = new File([blob], `${blob.name}.${new_type}`, {
          type: blob.type,
        });
        uploadImage(myImage);
      }, `image/${new_type}`);
  ...
  async function uploadImage(image) {
    ...
    if (url) {
      ...
      set_options({
        original_type:
          field.options.original_type || image.type.replace('image/', ''),
  ...
  <div class="image-info">
    ...
    <div class="inputs">
      ...
      {#if field.options.type && field.options.type !== 'svg+xml'}
        {@const original_active =
          field.options.type === field.options.original_type}
        {@const webp_active = field.options.type === 'webp'}
        <div class="image-type-buttons">
          <button
            on:click={() =>
              convert_image(field.value.url, field.options.original_type)}
