...
import stores, { timeline, update_timeline } from './data';
import {
  site,
...
export const symbols = {
  ...
  update: async (updated_symbol_id, updated_symbol_props) => {
    ...
    await update_timeline({
      doing: async () => {
        stores.symbols.update((store) =>
          store.map((symbol) =>
            symbol.id === updated_symbol_id
              ? { ...symbol, ...updated_symbol_props }
              : symbol,
          ),
        );
        dataChanged({
  ...
  rearrange: async (rearranged_symbols) => {
    const rearranged_symbols_with_indeces = rearranged_symbols.map(
      (symbol, i) => ({
        ...symbol,
        index: i,
      }),
...
export const active_page = {
  ...
  add_primo_block: async (symbol, position) => {
    ...
    await update_timeline({
      ...
      undoing: async () => {
        stores.symbols.update((store) =>
          store.filter((s) => s.id !== new_symbol.id),
        );
        stores.sections.set(original_sections);
  ...
  duplicate_block: async (block_id) => {
    const block = get(stores.sections).find(
      (section) => section.id === block_id,
    );
...
export const pages = {
  ...
  delete: async (page_id) => {
    ...
    await update_timeline({
      doing: async () => {
        ...
        // Delete child pages
        const child_pages = original_pages.filter(
  ...
  update: async (page_id, obj) => {
    const original_page = cloneDeep(
      get(stores.pages).find((page) => page.id === page_id),
    );
...
export async function update_section_content(section, updated_content) {
  const symbol = get(stores.symbols).find(
    (symbol) => symbol.id === section.symbol,
  );
  ...
  await update_timeline({
    doing: async () => {
      ...
      Object.entries(updated_content).forEach(
        ([language_key, language_content]) => {
          Object.entries(language_content).forEach(
            ([field_key, field_value]) => {
              const matching_field = symbol.fields.find(
      ...
      stores.symbols.update((store) =>
        store.map((s) =>
          s.id === symbol.id ? { ...s, content: updated_symbol_content } : s,
        ),
      );
      stores.sections.update((store) =>
        store.map((s) =>
          s.id === section.id ? { ...s, content: updated_instance_content } : s,
        ),
      );
      await Promise.all([
        dataChanged({
    ...
    undoing: async () => {
      stores.symbols.update((store) =>
        store.map((s) =>
          s.id === symbol.id ? { ...s, content: original_symbol_content } : s,
        ),
      );
      stores.sections.update((store) =>
        store.map((s) =>
          s.id === section.id ? { ...s, content: original_section_content } : s,
        ),
      );
      await dataChanged({
        table: 'sections',
