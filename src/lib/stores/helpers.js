import _, { chain as _chain, find as _find } from 'lodash-es';
import { get } from 'svelte/store';
import { processors } from '../component.js';
import { getEmptyValue, processCSS } from '../utils.js';
...
export async function buildStaticPage({
  ...
  const hydratable_symbols_on_page = page_symbols.filter(
    (s) =>
      s.code.js && page_sections.some((section) => section.symbol === s.id),
  );
  ...
  const component = await Promise.all([
    ...
    ...page_sections
      .map(async (section) => {
        const symbol = page_symbols.find(
...
export function get_content_with_static({
  component,
  symbol,
  loc = get(locale),
}) {
