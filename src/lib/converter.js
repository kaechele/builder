...
.content {
    ...
    const content = Object.entries(site.content).reduce(
      (accumulator, [locale, value]) => {
        ...
        page.fields.forEach((field) => {
          accumulator[locale][field.key] =
            value?.[page.id]?.[field.key] || getEmptyValue(field);
        });
        return accumulator;
      },
      {},
  ...
  const Section = (section, page) => {
    ...
    if (section.type === 'component') {
      ...
      content = Object.entries(site.content).reduce(
        (accumulator, [locale, value]) => {
  ...
  const pages = _.flatten(
    site.pages.map((page) => [
      Page(page),
      ...page.pages.map((child) => Page(child)),
    ]),
  );
  ...
  const sections = _.flatten(
    pages.map((page) => page.sections.map((s) => Section(s, page))),
  );
  ...
  const content = Object.entries(site.content).reduce(
    (accumulator, [locale, value]) => {
      ...
      site.fields.forEach((field) => {
        accumulator[locale][field.key] =
          value?.[field.key] || getEmptyValue(field);
      });
      return accumulator;
    },
    {
...
export function validate_symbol(symbol) {
  if (!symbol.content || _.isEmpty(symbol.content)) {
    ...
    const content = Object.entries(symbol.fields).reduce(
      (accumulator, [_, field]) => {
