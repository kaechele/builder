...
{#if !submitted}
  <main>
    <h1>Feedback</h1>
    <p>
      Let us know if something broke, if there's something you want to do that
      you can't do (yet), or even if you're just enjoying primo.
    </p>
    <form
      bind:this={form}
      on:submit|preventDefault={() => {
        ...
        axios
          .post('https://formspree.io/f/xknywogb', data)
          .then((res) => {
            console.log({ res });
            if (res.data?.ok) {
              successful = true;
...
{:else if submitted && successful}
  <main>
    ...
    <p>
      Thanks for your message! If you provided your email address, we'll try to
...
{:else if submitted && !successful}
  <main>
    <h1>Unsuccessful</h1>
    <p>
      For some reason we weren't able to submit your feedback. Please let us
      know by emailing contact@primo.af
    </p>
  </main>
{/if}
